...
本目录是 KeePasskey 项目的实现思路参考库，收录了 4 个开源密码管理器项目。**目录内各项目源码只读，仅作借鉴，严禁修改或将其代码复制入库**（许可证约束，详见各项目自带 LICENSE / license 文件）。
...
1. 实现某模块前，先查阅 `.codebuddy/skills/reference-projects.md` 中的参考项目地图，定位应参照的具体文件。
2. 再阅读本项目录中对应的架构分析文档，建立整体认识。
3. **只读约束**：任何操作不得改动本目录下 4 个项目目录中的任何文件；新增文档只允许放在本目录根下。
