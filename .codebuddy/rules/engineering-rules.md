...
- **命名有界**：避免无边界的 `Manager` / `Util` / `Helper` / `Common` 命名（防 God Util / 杂物间 Manager）。域前缀 + 单一职责的既有类型维持现名（见产品裁决 `PD-34` 白名单）；**新增**该族后缀默认红，优先改「域前缀 + 语义名」，确需保留后缀须同批扩 `tools/doc/check_bounded_type_names.py` 的 `ALLOWED` 并回写 `PD-34`。
...
- **设备侧用例必须实跑后才算入库（§150 立规，强制）**：`*/src/androidTest/**` 下**新增或修改**的用例，
  必须在真机 / AVD 上**实际执行该用例**（或如实登记「未执行」并说明原因）方可随批次入库；
  `compileDebugAndroidTestKotlin` 通过**不构成任何验证证据**——编译只保证类型自洽，不保证契约用对。
  （§150 实测教训：§149 新增 `PasskeyCreationDeviceTest` 时批次正文已如实标注「未跑设备侧」，
  该用例把**PEM 文本字节流**直接喂给契约入参为「原始标量 / 种子 / PKCS#8 DER」的 `signAssertion`，
  绕过生产断言路径的 `decodePemPrivateKeyText` 还原步骤 ⇒ **入库后首跑即红**。）
