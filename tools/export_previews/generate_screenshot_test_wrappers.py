...
# ISSUE-P3-318：原两处交替 / 可选重叠结构被 CodeQL 判 py/redos 指数回溯（high，#355/#356），分两步改写：
# ① ann 参数组 `(?:\((?:[^()\n]|\([^()\n]*\))*\))?` → `(?:\([^()\n]*(?:\([^()\n]*\)[^()\n]*)*\))?`
#    ——「非括号段」与「单层括号组」改为确定性串接（`(` 只能开组、组内只能是非括号字符），
#    任一位置只有唯一解析路径，回溯为线性；
# ② extra 中间注解行的 `(?:\([^)\n]*\))?[^\n]*` → `[^\n]*`——可选括号段被行尾 `[^\n]*\n` 完全覆盖
#    （语言冗余，删除后语言不变），消除「可选组 vs 行尾贪婪」的边界歧义；
#    中间注解行同时收敛为 `[ \t]*@[^\n]*\n`（量词-字面量-量词形态，无相邻重叠量词）。
# 两处均以「20,000 随机样本 + 生成产物逐字节 diff」验证等价（见批次 §327 / §328）。
