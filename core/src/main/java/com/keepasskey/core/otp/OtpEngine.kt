...
object Base32Decoder {
     ...
     * 解码 Base32 文本为字节流。
     *
     * **作用域声明（`ISSUE-P2-289` AC②）**：本解码器的宽容策略（忽略 `=` 填充、空白与
     * 字母表外字符）**仅服务存量库 TOTP 展示**——库内字段可能来自历史或第三方写入者，
     * 展示失败回退比静默更糟；本方法不抛出异常，空/无效输入返回空数组。
     * **新输入解析**（otpauth URI 录入 / 扫码 / 导入）**不得**经本宽容口径——其严格判据在
     * [TotpKeyUriParser]（含字母表外字符即解析失败），两个作用域互不外推。
     *
