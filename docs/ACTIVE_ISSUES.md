...
> **暂无开放项**（本区最近一次归零：§225 闭环的 `ISSUE-P2-217` / `ISSUE-P2-218` / `ISSUE-P2-219`
> ——fast-gate 的 `Android Lint` 无判定力、native-gate / device-gate 的 `Set up Android SDK` 恒失败、
> 依赖闸门 948 条未豁免达阈组合；实现与验收证据见
> [`docs/resolved/batches/225-CI门禁四项缺陷整改批次.md`](resolved/batches/225-CI门禁四项缺陷整改批次.md)）。
