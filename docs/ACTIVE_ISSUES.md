...
>
> **2026-09-16 用户裁决（对本节三条 P2：**本轮不考虑，仅留痕**）**：用户在获知各条的
> 代码取证与选项代价后裁决——**`ISSUE-P2-47` AC②、`ISSUE-P2-73` AC①、`ISSUE-P2-79`
> 本轮均不实施**（原话：「1，2，4，5 以上的内容均不考虑，先记录下来」）。逐条留痕如下，
> **三条仍留在表内**（属「已裁决暂缓」而非「已闭环」，以免日后误读为已处理）：
> - **`P2-47` AC②（封印载荷绑定库 header 摘要）**：不实施。依据是 §83 的代码取证——
>   `KdbxFile.kt:337/342` 保存时重新生成 KDF 盐与主种子，任何 header 摘要都会**每次保存都变**，
>   照原文实施将使**每次保存后快速解锁立即失效**。其 **AC③ 已完成**（§90 补齐引擎级回归，
>   含「重放被拒后缓存与基线不被覆盖」「上传遇重放拒绝合并」两例）。
>   「已见内容摘要链 + 状态缺失/MAC 失效 fail-open」的现状维持不变。
> - **`P2-73` AC①（按官方以 `EXTRA_AUTHENTICATION_RESULT` 回传数据集）**：不实施。
>   依据是 §84 记录的**硬冲突**——要让框架真正写入数据集，认证 Activity 须拿到字段 `AutofillId`，
>   其正规途径是框架注入的 `EXTRA_ASSIST_STRUCTURE`，而**该注入要求 PendingIntent 为
>   `FLAG_MUTABLE`**，正是第四轮定版批注**明令不得改**的那一项（且本仓这两条路径不消费 fillIn extras）。
>   本批已完成的**崩溃级修复**（Android 12+ 要求非空 extras）保持有效，AC② 依批注不实施。
> - **`P2-79`（KDF 强度基线口径）**：不实施。建库默认维持 `Argon2id m=64 MiB / t=2 / p=2`；
>   不接入 `KdfBenchmark` 建议值（该 AC 原文即标注「需产品确认」，且其 AC② 含**降强陷阱**：
>   直接接入建议值会出现 `8 MiB < 64 MiB`）。
> - **`P3-121`（自建内网 WebDAV / NAS）**：不实施（登记于 P3 节前言）。
...
- **2026-09-16 复核批注（§83 批次，实施前必读）**：
  ...
  - **AC③ 已完成（§90 批次；含一处前提更正）**：`SyncRollbackGuardTest` 已有 8 例（摘要链、
    曾接受版本判重放、F-23 状态存活、fail-open 口径、状态不被缓存清理删除）。
    §83 批次的复核批注曾记「**尚缺引擎级『远端回退 → 拒绝且不覆盖本地』用例**」——
    该前提**已部分不成立**：`SyncEngineTest` 早在 `ISSUE_P2_18` 批次就有
    `被入侵端点重放旧库被拒绝应用`（断言 `RollbackRejected`）。**真正缺的是后半句**：
    该用例只断言裁决与返回字节，**未断言本地缓存 / 基线未被覆盖**——即「拒绝」的全部意义所在。
    §90 据此补齐两例：`重放被拒后本地缓存与基线不被覆盖`（无守卫时 `openRemote` 会把重放内容写进
    缓存与基线并**看起来同步成功**，故该断言同时在做负向鉴别）与
    `上传路径遇远端重放拒绝合并并保留本地`（覆盖此前**完全无用例**的 `SyncCommitResult.RollbackRejected`
    分支，即「冲突下载到的远端内容恰是重放 ⇒ 不得进入三方合并」，否则被命中的旧库会把已删条目复活）。
...
>
> **2026-09-16 闭环（§91 批次）**：**`ISSUE-P3-120`**（`RuntimeIntegrityDetector` 拦截力无实测）
> 已收口——**真机三形态实测命中率 3/3**（用户已授权联网取件与装入真机）：
> ① **默认落点**：`/data/local/tmp/frida-server` **仅在案、未运行**即判 `COMPROMISED`；
> ② **改名**：服务端改名后**落点层漏报**（等级仍 `ELEVATED`），但 attach 后
> `/proc/self/maps` 出现 `/memfd:frida-agent-64.so (deleted)` ⇒ **maps 层命中** `COMPROMISED`；
> ③ **内存加载**：同一次实测中 agent 本就是以 **memfd** 载入、磁盘零落点，仍被 maps 层命中。
> **AC② 结论为「不需要加强检测」**（无实测证据支持），改按 AC 的「或如实声明」分支，
> 把实测矩阵与**四条未经实测的规避面**写入 `detectHookFramework()` 的 KDoc 与
> [records 实测基线](records/运行完整性检测Frida实测基线.md)。
> **两层防线互补**（只留落点层则改名即绕过；只留 maps 层则「落点已就位未注入」不可见）是本次
> 实测最实质的产出。设备已清理（服务端进程终止、落点删除，复测回基线）。
> 见 [RESOLVED_LOG.md](RESOLVED_LOG.md) §91。
>
> **2026-09-16 用户裁决（`ISSUE-P3-121`：本轮不考虑，仅留痕）**：用户在获知选项代价后裁决
> **不给出厂可用口径**——`SyncNetworkOptions.ssrfAllowedHosts` **维持默认空集**，
> 即自建内网 / `.local` 主机仍一律被 `SyncEndpointGuard` 拒绝、无生产逃生通道。
> 理由（据该项正文）：这是 SSRF 纵深防御的**默认收紧**，放开等于给「同步端点被指向内网」留口子；
> 需要内网同步的用户可经既有显式白名单机制自行配置（该项 AC① 的「明确产品口径」即为本裁决）。
> **该条仍留在表内**（已裁决暂缓，非已闭环）。
>
> **2026-09-16 闭环（§97 批次）**：**`ISSUE-P3-132`**（外部 UI 评审「边界定义 / 弹窗规范 / 竖向空间」，
> 同轮登记并闭环，不在表内留存行）——核实先行（预览图逐像素取样 + material3 源码核对 + 全仓 `grep`）
> 判定 **5 类主张中 3 类不成立**并逐条留痕；成立项全部整改：13 处弹窗 `shape` 覆盖删除（回到 MD3
> `CornerExtraLarge`）、`outline` 语义色由半透明改不透明（OutlinedTextField 未聚焦边框 **1.26:1 → 4.26:1**）、
> 禁用态主按钮补可见边界（**1.28:1** ⇒ `surfaceContainerHighest` + 1dp `outline`）、详情页快捷操作改紧凑
> `AssistChip` 行、长路径改中段省略（`middleEllipsize`）；新增 14 例回归（含全仓 AlertDialog 调用点扫描）。
> 见 [RESOLVED_LOG.md](RESOLVED_LOG.md) §97。
...
> （来源 SECURITY_RECHECK §11，附核实行号）。原表头（`| 编号 | 来源 | 问题与位置 | 验收标准 |`
> 及其分隔行）下方**无任何数据行**——该批次的唯一低危行早已闭环归档，表头属遗留残壳，
> 2026-09-16 一并删除（无信息丢失）。

---

> **2026-09-16 新增（§102 施工中的附带发现，**开放项**）**：下表 1 项由「第四轮复核工作目录
> 分批判定 → 最终报告」的**ID 级对拍**发现。§102 批次已据此撞出并修复两条真实缺陷
> （`ISSUE-P2-85` 建库算法丢弃、`ISSUE-P3-140` 末尾残块明文副本未清零），
> 但**其余判定的逐条复核未完成**，故登记为开放项（**不伪闭环**）。

| 编号 | 来源 | 问题与位置（核实于 2026-09-16，对 HEAD `ba11edc`） | 验收标准 |
|---|---|---|---|
| ISSUE-P3-141 | 第四轮复核工作目录（`.audit-recheck/`，`.gitignore` 排除） | **分批判定的处置状态不可核对**：以正则对拍「工作目录 `B01`~`B09` 分批判定文件」与「`SECURITY_RECHECK_2026-09.md` + `ACTIVE_ISSUES.md` + `RESOLVED_LOG.md`/批次正文」的全部编号，得 **22 个编号仅存在于工作目录**（`ANDROID-01/06/10`、`ISSUE-P1-04/05`、`ISSUE-P2-08`、`ISSUE-P3-25/40`、`NEW-B01-1/5/6`、`NEW-B02-1/5`、`NEW-B04-01/03/04/05/06`、`NEW-B09-01~05`）。**已核实为真实缺口**：`NEW-B02-5`（建库预设算法与 `createDefault` 默认值不一致）与 `NEW-B02-3`（末尾残块明文副本）均属真缺陷，却**从未进入报告或本表**；报告 §6.8 只逐条裁定 `IPC-*`/`SUPPLY-*`，`NEW-B*` 仅少量被 §8 根因矩阵引用 | ① 对上述 22 个编号逐条给出「已由某条目覆盖 / 已在本批修复 / 确属新缺陷 / 前提不成立」四选一结论**并落到文档**（不得只留工作目录）；② 确属新缺陷者登记到本表；③ 结论须与 `SECURITY_RECHECK_2026-09.md` 交叉引用，使「复核未遗留未登记缺陷」从**断言**变成**可核对**；④ 同时评估是否把工作目录的关键分批判定（`.gitignore` 排除、不随仓库分发）按退役纪律分流入库 |
| ISSUE-P3-142 | 全量单测偶发（§79 残余的**第二次观测**，来源非本仓源码） | **`SyncCacheTest.\`clear 与 clearAll 均不删除防回滚状态文件\`` 偶发失败**：清理后目录内残留 `<大写十六进制键>.CACHE.tmp`（该键即 `sha256("remote/vault.kdbx")`，本次实测值 `8215950248E31E…BED990`）——**该命名无法从本仓源码复现**：`SUFFIX_CACHE = ".cache"`（小写）、键由 `SyncCache.sha256Hex`（`digest.toHexString()`）产出（小写）。**核实（2026-09-16）**：① 全量 `.\gradlew.bat test --rerun-tasks --max-workers=1` 复现一次（**当天同日另两次全量运行均通过**）；② 单独复跑 `:sync:testDebugUnitTest --tests "*SyncCacheTest*"` **连续 3 次全绿**；③ 该症状与 `RESOLVED_LOG.md` §79.5 第 1 条**逐字一致**（§79 已登记「若后续再次观测到大写命名残留，应作为独立线索排查（可能来自本仓之外的写入者）」，本次即第二次观测）；④ §79 的「有界重试」修复（`deleteCacheChild` 3×15 ms）**未能覆盖本形态** | ① 定位该大写命名条目的**实际写入者**（在本仓源码、Gradle/JUnit 临时目录机制、外部进程三者中给出结论，附取证方式）；② 定位后给出处置：属本仓者修根因并补回归，属外部写入者则在 §79 的登记处补「已确认为外部来源」及规避方式；③ **定位前不得放宽该断言**（保持「清理后不得残留」的严格语义）；④ 若长期不可复现，按「已接受残余」登记并写明触发条件与复现尝试次数 |
