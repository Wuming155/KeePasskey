...
- **剩余清单（本条尚未闭环的部分，逐条自包含）**：
  ...
  1. **Compose 面的长函数**（第 3 目的剩余部分）：**§207 现跑 = 12 个 ≥100 行函数**（§200 现跑 18；
     §205 拆出 `PackageBlocklistManageDialog` 131 与 `GeneratorContent` 121、§206 拆出
     `BasicCredentialsCard` 118 与 `SyncStatusCard` 119、§207 拆出 `themeListSection` 118 与
     `VaultDatabaseCard` 117，逐批留痕见 [`resolved/batches/205-Compose长函数拆分两处批次.md`](resolved/batches/205-Compose长函数拆分两处批次.md)、
     [`resolved/batches/206-Compose长函数拆分两处批次.md`](resolved/batches/206-Compose长函数拆分两处批次.md)
     与 [`resolved/batches/207-Compose长函数拆分两处批次.md`](resolved/batches/207-Compose长函数拆分两处批次.md)），
     扣除 2 个 PD-11 豁免 NavGraph ⇒ **待拆 10**。
     逐个名单与已出表 9 个的逐批留痕见
     ...
     **当前表内头部**（现跑读数）：`CreateVaultWizardDialog` 158（**受限界 §179 下限**——主密码
     `CharArray` 擦除链的「单一现场」决定，勿再拆）、`EntryDetailScreen` 121、`HealthCheckScoreCard` 116、
     `MasterKeyChangeDialog` 116、`ChildVaultEntryRowView` 113。
