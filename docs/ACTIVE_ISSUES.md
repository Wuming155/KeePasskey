...
- **环境可得性复核（2026-09-25，§317~§323 存量整改会话）**：本机 `docker` 不存在、无本地 `httpd` / `nginx` 安装、IIS 可选功能查询不可用（需管理员）⇒ 四类服务器矩阵均无真实环境；`tools/local-sync` 既有口径不产 ETag 仍不构成弱面取证。按 AC③/AC④ 如实标注**未执行**，本条维持开放。
