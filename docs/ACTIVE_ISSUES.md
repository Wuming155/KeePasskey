...
- **设计交付（AC①，2026-09-20）**：[`architecture/敏感缓冲所有权契约.md`](architecture/敏感缓冲所有权契约.md)
  ...
  **收口点登记表**（15 个 O/S 类收口点 + P 类清单，逐个附擦除钩子与守卫用例）、
  **非秘密数据豁免下界清单**（`E1` ~ `E8` 逐项给理由 + 反例清单）、
  `已知工程限界.md` **§1.6 池内擦除解除条件评估**、迁移路线图（Step 0 ~ Step 4）与
  §9.6 七条红线**逐条对照表**（该文档 §7.1）。**本次交付不改动任何生产代码**
  （AC② 明令「设计评审通过后再迁移」）。
- **验收标准**：
  - AC① ✅ **已交付**（见上，指向设计文档）；
  - AC② **待设计评审后逐处迁移**，禁止一次性大改；`§1.6` 池内擦除的解除条件评估**已在设计 §6 完成**
    ——结论为「规则文本前提已满足（`BinaryItem` 类别已可判定），但**实施**仍须先满足准入
    ①`G2` 别名持有者全部纳入会话终止三事件（`RC-05` 归口）、②池内擦除必须走身份集合判定，
