...
> 供应链告警豁免（`ISSUE-P3-321`）与 Dependabot 积压 PR 验证合并（`ISSUE-P3-322`）闭环见 §330；
> 动效节奏降档复定标（`ISSUE-P3-323`）闭环见 §331；
> 无障碍假提示摘除与旧版无障碍填充通道落地（`ISSUE-P3-324`）闭环见 §332；
> 运行完整性门整体移除（`ISSUE-P3-325`，PD-45 推翻 PD-13 / PD-15）闭环见 §333。
>
> **全量待办清零（P0 → P3）。**
