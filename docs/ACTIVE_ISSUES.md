...
- **2026-09-11 补记（重要反证，需重测）**：同批次**附件导出场景**中，SAF 选择器（DocumentsUI）
  前台约 10 秒后返回 App，**库已被自动锁定**（落在解锁页）——说明「其他 App 占前台」时锁定可能生效，
  与 HOME 场景的「不锁定」相矛盾。HOME 触发路径（`keyevent 3` 是否真使进程 `onStop` /
  `ProcessLifecycleOwner` 判后台）需用 `dumpsys activity processes` 观察 lifecycle 状态重测后再定论。
...
### ISSUE-P2-20: 附件「导出到存储」经 SAF 保存后产物为 0 字节（**待复核**）

- **优先级**：P2（导出功能产物损坏；源附件本身完好，无数据丢失）
- **核实时间点与核实方式（2026-09-11）**：AVD `Pixel_10` 实操：条目编辑页经 SAF 选择器添加
  `keepasskey-attach-test.txt`（9.51 kB，`adb push` 到 `/sdcard/Download/`）→ 保存 →
  详情页「附件与证书文件」出现该附件 → 点「导出到存储」→ SAF 另存对话框预填同名文件名 →
  点「保存」。结果 `/sdcard/Download/` 出现 **`keepasskey-attach-test.txt (1)`，大小 0 字节**
  （`adb shell ls -l` 取证）；原文件 9513 字节未受影响。全程 logcat 无 FATAL。
- **疑点**：`(1)` 后缀为 SAF 对同名冲突的自动改名，说明「创建文档」成功但**内容未写入**（或写入流
  未关闭/未 flush）。可能是 App 侧导出协程在 SAF 返回后未完成写入，也可能与本会话中途的
  自动锁定（库会话熔断，见 ISSUE-P1-14 的 SAF 前台场景补记）打断了写入。
- **待复核方式**：锁定偏好全关的状态下重做一次附件导出，检查产物大小与内容（`md5` 对比源文件）；
  若仍为 0 字节则确认为导出实现缺陷。
- **验收标准（待整改）**：① 导出产物与源附件逐字节一致；② 导出失败时给出可见错误而非静默产出空文件。
