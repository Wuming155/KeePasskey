...
> **2026-09-09 TASK-52**：解锁性能提速——引入自维护 Argon2 原生构建（PHC 官方参考实现 + 薄 JNI 桥）优先派生，BC 降为兜底；旧版 KDBX 错误文案简化；BC 兜底路径补内存参数预检与 OOM 友好失败（防闪退）。
