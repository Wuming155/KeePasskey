...
自动填充认证结果崩溃级修复与 requestCode 单调化批次（依定版要求 `P2-73` 与 `IPC-01` **同批**）：官方原文证实两处裸 `setResult(RESULT_OK)` 是 **Android 12+ 崩溃级缺陷**（minSdk 36 恒在区间），按官方等价做法改为 `setResult(RESULT_OK, Intent().putExtras(Bundle.EMPTY))`；`IPC-01` 认证 requestCode 由常量改为**进程级单调分配器**（与 CM 同构），消除「共用 requestCode + `FLAG_UPDATE_CURRENT` ⇒ 点旧候选拉起新上下文」的 TOTP/授权串扰面。**两条目均未闭环**：`P2-73` AC② 依批注不实施、AC① 的「数据集回传」与 AC② **硬冲突**（需框架 fillIn ⇒ 必 MUTABLE）已登记为决策点、AC③ 需真实 autofill 客户端；`P3-122` IPC-10 未做
