...
### 3.3.1 §324 回填：矩阵实测已完成（Apache/mod_dav + nginx-dav）

`ISSUE-P3-300`（§324）于本机自建真实服务器（Apache 2.4.68 + mod_dav/fs/lock、nginx 1.30.5 dav
模块）完成 AC① 三项读数与 AC⑤(a) 实验。关键读数：**mod_dav 本身即弱 ETag 签发服务器**
（`W/"size-mtime"`），`If` 头原形态回传（弱→弱）实测 **201 接受**、错值 412、强化形态 400
响亮失败；nginx 对 MOVE 的 `If` 整体忽略（无 412 误报）；`Overwrite:F` 在两类服务器均被真实
尊重（412）⇒ 404 恢复分支 TOCTOU 非真实残余。「回传原形态」取向经 AC② 证实分支闭环；
Nextcloud / IIS / S3 类样本未执行（环境不可得，如实标注）并收窄登记入限界表 §28。
读数全文见 [`324-真实DAV矩阵实测批次.md`](324-真实DAV矩阵实测批次.md) §2。
