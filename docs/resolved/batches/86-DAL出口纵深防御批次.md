...
DAL 出口纵深防御批次：DAL 出口客户端不再由校验器自建，改由 `DalVerifierModule` 以 `@DalHttpClient` 提供并统一走 `SyncHttpClientFactory` ⇒ 获得 **TLS-only**（显式排除 CLEARTEXT）与 **`SsrfGuardDns`**（连接期拦截内网/保留网段 + 抵御 DNS 重绑定）两项加固，`ssrfAllowedHosts` 保持空集（内网 WebDAV 逃生通道不适用于 DAL）。AC② 断言**对生产提供方法求值**而非测试替身（逻辑用例面向明文 MockWebServer，加固客户端必然拒绝它们，两者不可共存）；AC④ 两侧 skip 开关不对称的**留痕结论为「有意设计」**（自动填充侧判域归属，给跳过开关是放松放行面而非隐私让步）
