...
1. **残留条目的命名与源码不一致，未能定位其来源（如实登记）**：该次残留名为
   ...
   > **2026-09-16 就地标注结案（`ISSUE-P3-142`，`RESOLVED_LOG.md` §107；原句按归档纪律保留）**：
   > 该线索**已结案**——大写条目在**磁盘上并不存在**（四路交叉取证一致否认：`exists/isFile/canRead/length` 全 false/0、
   > NIO `readAttributes`/`DirectoryStream` 读不到、原生 `cmd dir /b` 看不到），**本仓源码与外部进程均非写入者**；
   > 判为 **Windows/NTFS 目录枚举「鬼影条目」**（机制未证实）。断言侧规避方式为「**以磁盘真实存在为准**」判定残留
   > （真残留仍必红），已应用于 `SyncCacheTest` 与同族 `SyncCacheEvictorTest`；残留约 **3%** 假阳性与
   > `deleteOrphanTmpFiles` 大小写敏感匹配两项已登记为**已接受残余**（`docs/architecture/已知工程限界.md` §7）。
