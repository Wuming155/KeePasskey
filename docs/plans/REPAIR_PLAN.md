...
> **更新时间**：2026-09-08（TASK-47 落地，规划段落删除；余 5 项）
> **文档定位**：本文档为 `STATUS.md` §2「未完成工作唯一看板」中剩余 **5 项未实现 / 待验证任务**（TASK-02 / 19 / 43 / 48 / 49）的详细落地规划，**仅作执行依据**；各任务的「状态 / 优先级」以 `STATUS.md` 看板为唯一真相源，本文档不维护状态列。**TASK-44（2026-09-08）、TASK-46（2026-09-08）、TASK-45（2026-09-08，`d4bc46c`）、TASK-20（2026-09-08，`c530761`）、TASK-47（2026-09-08）已落地并回写 `STATUS.md` §2，其规划段落随之删除**。
...
| 4 | **TASK-48** | 数据 | 占位库假元数据（P2-26） | ❌（风险已接受） | P3 | 保留引导则须 UI 显式区分 |
| 5 | **TASK-49** | 特性 | 图标渲染 / 删除 + 引用展示侧残余接线 | ❌ | P3 | 数据通道已就绪，消费点平移 |

> 上表「STATUS 状态」为 2026-09-08 快照引用，实时状态一律以 `STATUS.md` §2 为准。TASK-44 / TASK-47 已于 2026-09-08 落地并随代码提交回写 `STATUS.md` §2，规划段落已删除；TASK-45 / TASK-20 此前已落地（见各自提交 `d4bc46c` / `c530761`）。
...
1. 每批落地后 `.\gradlew.bat test` 全绿（当前基线 506 例：494 通过 / 12 跳过）方准入库。
