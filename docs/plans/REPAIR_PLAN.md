...
> **更新时间**：2026-09-08（看板口径校准：补登 TASK-47~49）
> **文档定位**：本文档为 `STATUS.md` §2「未完成工作唯一看板」中剩余 **6 项未实现 / 待验证任务**（TASK-02 / 19 / 43 / 47 / 48 / 49）的详细落地规划，**仅作执行依据**；各任务的「状态 / 优先级」以 `STATUS.md` 看板为唯一真相源，本文档不维护状态列。**TASK-44（2026-09-08）、TASK-46（2026-09-08）、TASK-45（2026-09-08，`d4bc46c`）、TASK-20（2026-09-08，`c530761`）已落地并回写 `STATUS.md` §2，其规划段落随之删除**。
...
| 4 | **TASK-47** | 安全 | 已泄露密码检测接 HIBP（P2-28） | ❌ | P3 | 先做「接 / 下架」决策再动手 |
| 5 | **TASK-48** | 数据 | 占位库假元数据（P2-26） | ❌（风险已接受） | P3 | 保留引导则须 UI 显式区分 |
| 6 | **TASK-49** | 特性 | 图标渲染 / 删除 + 引用展示侧残余接线 | ❌ | P3 | 数据通道已就绪，消费点平移 |
...
## TASK-47（P3）：已泄露密码检测接 HIBP（FINDINGS P2-28）

**依据**：`FINDINGS_TRACKER.md` P2-28；`STATUS.md` §2 TASK-47。
**背景**：健康度面板 `compromisedPasswordCount` 硬编码为 0，属「假指标」（与已整改的 TASK-32 强度恒 112 / TASK-33 TOTP 假码同类问题）。

**前置决策（必先裁定）**：本项目定位为「离线优先、零外联」，接入 HIBP 会引入外部网络依赖。
- **路线 A（接入）**：k-anonymity 范围查询——仅上传密码 SHA-1 前 5 位前缀，明文与完整哈希不出端；必须显式开关（默认关）+ 失败如实上浮（不得静默回落 0）。
- **路线 B（下架）**：若裁定不引外部依赖，按 TASK-12/13/36 诚实化先例**删除该指标与其 UI 展示**，而非显示恒 0。

**风险**：外部服务可用性与隐私声明（Play 数据安全表单需更新）；k-anonymity 仍存在前缀关联的信息泄露上界，需在文案中如实说明。
**验收**：路线 A——前缀查询回归（含 mock 服务端）+ 关闭态不发起任何网络请求断言；路线 B——指标与字符串资源零残留。
