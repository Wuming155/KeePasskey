...
| **P3-27** | 空 onClick 按钮与写死黑名单 | ✅ 已修复（2026-09-08；完整生命周期随 TASK-44 于 2026-09-08 闭合） | `AutofillSettingsScreen` 黑名单对话框移除写死示例条目与空 onClick 删除按钮，改诚实展示真实计数/空态；孤儿字符串（中英 4 条）清理；完整生命周期登记 TASK-44。**TASK-44 落地**：新增 `AutofillBlocklistStore`（包名集合持久化 + `blockedPackages` Flow + Android 官方包名校验），`KeePasskeyAutofillService` 与 `KeePasskeyCredentialProviderService` 双通道命中即 fail-closed 不下发；设置页改条目化列表（应用名+包名）与删除、按包名新增（非法/重复如实报错）；详情页 `android://` 绑定条目提供「为本应用禁用自动填充」入口；无写入方的 `disabledAutofillQueriesCount` 计数及其持久化键一并下架 | 中（功能） | 沿 TASK-12/13 诚实化裁定先例；假功能不再回显，黑名单端到端闭环 |
