...
| C-10 | 分支保护是否要求 `build` 工作流通过（决定 `F-05` 的爆炸半径） | `gh api repos/{owner}/{repo}/branches/main/protection`；**⇒ 已答（2026-09-17）**：`main` **无任何分支保护规则**（仓库所有者于 GitHub 设置页一手核实，见 [`architecture/产品裁决登记.md`](../architecture/产品裁决登记.md) `PD-04`）⇒ **`build` 并未被要求通过**，`F-05` 的爆炸半径**不因分支保护而收窄** |
