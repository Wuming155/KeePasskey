...
| `[D]` | 设备侧 / 真机实测 | ✅ —— 报告发布时未使用；`P3-120` 已于 2026-09-16 补测（见 §10.1） |
...
1. **未做设备侧实测**（委托方裁决）。故 `P2-42`、`P3-82`、`P2-74`、`P2-63`、`P2-73`、`P2-44`、
   ...
   **状态更新（2026-09-20，`ISSUE-P3-233`）**：`P3-120` 已于 2026-09-16 真机三形态补测 **3/3 命中**（`[D]`，见 §10.1），
   **不再是未验证项**；其余各项状态见 §10.1。
...
| `P3-120` | P3 | MEDIUM（潜在） | **已完成（`[D]` 实测）** | P1 | `[D]` 2026-09-16 真机三形态 **3/3 命中**（Redmi 4X / LineageOS / Android 17 / API 37，frida-server 17.15.3）⇒ **原「多条门控的共同前提」已由实测证成**（`P2-53`/`P2-63`/`P2-76`/`P1-23`）；证据见 §10.1 |
...
| **`NEW-CHAIN-2`**（`AC-09`） | `P2-53` + `P2-63` + `P2-76` + `P3-120` | ⚠️ **条件连通** | 已 root 设备上 CM 通道**完整性裁决 + 用户验证两层同时可绕** | **已实测 3/3（2026-09-16，`[D]`）⇒ 该前提成立**（原写「取决于 `P3-120` 实测命中率」）；**`P2-53` / `P2-63` 互为前提** |
...
| P1-7 | `P3-120` | 真机以 Frida 三种形态实测命中率并写入设备侧基线；**它是 `P2-53` / `P2-63` / `P2-76` / `P1-23` 的共同前提**。**已完成（2026-09-16 实测 3/3，`[D]`，见 §10.1）** |
...
| ~~`P3-120`~~ **已完成（`[D]`）** | 真机 Frida 三形态（默认 / 改名 / 内存加载）命中率 —— **2026-09-16 实测 3/3 命中**（Redmi 4X / LineageOS / Android 17 / API 37，frida-server 17.15.3）。证据：`RuntimeIntegrityDetector.kt:216-229` KDoc + [`docs/records/运行完整性检测Frida实测基线.md`](../records/运行完整性检测Frida实测基线.md) |
...
11. **【P0 判据·锁定，防未来摇摆】DoS / 可用性类条目必须过两道检验**（第三次审核确认后固化）：
    ...
    12. **【2026-09-20 立规·`ISSUE-P3-233`】引用任何复核结论前，必须对拍被测代码 / 产物的 HEAD 现状**：
    本报告正文是**发布时点的快照**，条目状态会随后续修复批次漂移；直接引用旧结论而不核对现状，
    会让评审双方**同时基于陈旧前提推演**。**实例（本轮真实情节）**：`P3-120` 在 §3.3.2 矩阵标 `[U]`、
    §10.1 列为「需设备侧证据」，而 HEAD 的 `RuntimeIntegrityDetector.kt:216-229` KDoc 已明载
    **2026-09-16 真机三形态 3/3 命中** —— 2026-09-20 的过度安全设计评估与三轮审计中，评审双方
    连续两轮引用该陈旧状态（`SYS-5`「基线与 HEAD 混用」的典型复发），直至补读被测代码才瞬时收敛。
    **可执行判据**：安全结论的引用处须与 `[V]` 证据级同构地给出「被测代码路径 + HEAD 核对方式 + 核对时点」。
