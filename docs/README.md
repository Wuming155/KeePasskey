...
| [`records/限界表证据与实测数据汇录.md`](records/限界表证据与实测数据汇录.md) | [`architecture/已知工程限界.md`](architecture/已知工程限界.md) 的**证据 / 实测 / 沿革附卷**（§253 批次外迁）：按 `§x.y` 分节收录从限界表移出的实测表、多轮叙事、根因推导与观测点记录（**逐字搬移、不改写**），另含原表文末「附」表 40 行的逐字归档；限界表内相应位置留一行指针 |
