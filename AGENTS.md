...
| 维度 | 数值 / 状态 | 官方依据与说明 |
|---|---|---|
| **Git HEAD** | 代码基线 `9c2a806` → **P3 残余批次 12 项整改**（ISSUE-P3-17 ~ P3-28：**8 项完整闭环 + 4 项部分达标**；含 1 处全框架致命缺陷、9 处生产代码缺陷、15 条过程缺陷与事实修正如实留痕） | 归档见 [RESOLVED_LOG.md](docs/RESOLVED_LOG.md) **§4**；4 项残余面就地更新为 ISSUE-P3-20 / P3-23 / P3-24 / P3-25 |
| **测试基线** | **1159 个单元测试用例**（app 637 / core 58 / crypto 61 / database 224 / sync 179）：**1146 通过、0 失败、13 跳过**（基线 921 → **+238 例，零退化**）（另有 Rust 侧 `cargo test` 9 例，见 §5；crypto 另有 7 例 instrumented 测试，见下） | `.\gradlew.bat test --rerun-tasks --max-workers=1` 强制真实执行全模块；13 例跳过为 `LiveSyncServersTest` 真实联调用例（12 例，需先起 `tools/local-sync` 服务并加 `-DliveSyncTest`）+ `SyncCacheTest` 的 Windows 无 POSIX 权限视图断言（1 例） |
...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test`；当前 **1159 例：1146 通过 / 0 失败 / 13 跳过**，分布 app 637 / core 58 / crypto 61 / database 224 / sync 179，其中 12 例跳过项需 `-DliveSyncTest` 才启用，1 例为 Windows 无 POSIX 权限视图的缓存权限断言）
