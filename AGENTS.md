...
| **测试基线** | **573 个单元测试用例**（app 185 / core 32 / crypto 54 / database 160 / sync 142，其中 sync 142 含 12 例联调跳过）：**561 通过、0 失败、12 跳过** | `./gradlew test` 全模块执行；跳过的 12 例为 `LiveSyncServersTest` 真实联调用例（需先起 `tools/local-sync` 服务并加 `-DliveSyncTest`） |
...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test`；当前 **573 例：561 通过 / 0 失败 / 12 跳过**，分布 app 185 / core 32 / crypto 54 / database 160 / sync 142，跳过项需 `-DliveSyncTest` 才启用）
