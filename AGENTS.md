...
| **测试基线** | **532 个单元测试用例**（app 172 / core 32 / crypto 52 / database 155 / sync 121，其中 sync 121 含 12 例联调跳过）：**520 通过、0 失败、12 跳过** | `./gradlew test` 全模块执行；跳过的 12 例为 `LiveSyncServersTest` 真实联调用例（需先起 `tools/local-sync` 服务并加 `-DliveSyncTest`） |
...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test`；当前 **532 例：520 通过 / 0 失败 / 12 跳过**，分布 app 172 / core 32 / crypto 52 / database 155 / sync 121，跳过项需 `-DliveSyncTest` 才启用）
