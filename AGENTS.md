...
- 测试 / 构建 / CI 当前全绿（具体版本、例数、残余面见 [`RESOLVED_LOG.md`](docs/RESOLVED_LOG.md)）。
  单测基线（2026-09-15，§73 / §74 批次后）：**1788 例 / 0 失败 / 0 错误 / 13 跳过**
  （app **991** / core 68 / crypto 131 / database 392 / sync **206**；§73 新增导出面守卫 5 例、
  §74 新增清理幂等回归 3 例；
...
| [`docs/resolved/`](docs/resolved/) | **历史批次归档**：4 份分册索引（各 ≤100 行；分册 04 为 **§58 起滚动册**）+ `batches/` 下**一批次一文件**的正文（72 份；§42 / §43 两份按「退役承接」体例存于 `docs/security/`） | 查 §1 ~ §74 任一批次的验收证据 / 裁决 / 过程缺陷 |
