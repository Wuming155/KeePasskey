...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test`；当前 **1328 例：1315 通过 / 0 失败 / 13 跳过**，分布 app 749 / core 58 / **crypto 107** / **database 235** / sync 179，其中 12 例跳过项需 `-DliveSyncTest` 才启用，1 例为 Windows 无 POSIX 权限视图的缓存权限断言）
  > 数值口径修正（2026-09-10 实测）：此处原记 1291 例 / app 712（更早曾记 1257 例 / app 678，均系快照滞后）；
  > 现按各模块 `build/test-results/testDebugUnitTest/*.xml` 实测汇总为 **1328 例 / app 749**，与 §1 基线一致
  > （较 1291 净增 37 例，全部为 ISSUE-P3-31 批次 C + P3-43 批次新增单测，见 RESOLVED_LOG §11.6）。
