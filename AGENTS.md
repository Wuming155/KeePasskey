...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test` 已就绪，`testDebugUnitTest` 可单模块执行；当前 173 个测试全绿）
- 版本升级需整体配套：AGP ↔ Gradle ↔ Kotlin ↔ Compose BOM（Compose BOM 2026.06.00+ 要求 compileSdk 37，当前用 2026.06.01 对齐 compileSdk 36）

**当前阶段状态**：**🔧 参考项目差距修复专项（`REMEDIATION_PLAN.md`）Wave 1-6 全量交付完毕（Wave 6 = 全面安全审查整改专项）。**
...
**测试基线**：全工程 173 个单元测试全绿（app 66 / core 9 / crypto 34 / database 31 / sync 33）；`assembleDebug` 与 `assembleRelease`（R8 混淆）构建闭环通过。
