...
| 文件 | 内容 | 何时阅读 |
|------|------|----------|
| [**docs/STATUS.md**](docs/STATUS.md) | **单一真相源**：当前版本基线、未完成任务唯一看板（46 项：42 ✅ / 2 📋 / 2 ❌）、历史提交日志索引 | **开始任何工作前、检查进度时** |
...
- `.\gradlew.bat test` — 单元测试（全模块 `src/test`；当前 **475 例：463 通过 / 0 失败 / 12 跳过**，跳过项需 `-DliveSyncTest` 才启用）
