 ...
 *   用户如确有离线注册需求，可经设置页「跳过通行密钥站点归属校验」（`skipDalVerification`，
 *   默认关闭）**显式**授权降级——该开关是用户主动操作，满足「显式用户告警/授权」替代路径，
 *   且落告警日志。该开关的**界面文案与本段措辞互为锚点**（`ISSUE-P2-240` 前的界面文案误写为
 *   「跳过浏览器兼容层」，与该开关的真实语义无关；`ISSUE-P2-228` 同类缺陷），由
 *   `DalSkipSwitchCopyWiringTest` 机检锁定，任一侧改写而未同步另一侧即红。
